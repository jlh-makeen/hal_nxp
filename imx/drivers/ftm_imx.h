...
#define FSL_FEATURE_FTM_CHANNEL_COUNTn(base) 8
...
{
    kFTM_NoOutputSignal = (1U << FTM_CSC_MSA_SHIFT), /*!< No channel output when counter reaches CnV  */
    kFTM_ToggleOnMatch  = ((1U << FTM_CSC_MSA_SHIFT) | (1U << FTM_CSC_ELSA_SHIFT)), /*!< Toggle output */
    kFTM_ClearOnMatch   = ((1U << FTM_CSC_MSA_SHIFT) | (2U << FTM_CSC_ELSA_SHIFT)), /*!< Clear output */
    kFTM_SetOnMatch     = ((1U << FTM_CSC_MSA_SHIFT) | (3U << FTM_CSC_ELSA_SHIFT))  /*!< Set output */
} ftm_output_compare_mode_t;

/*! @brief FlexTimer input capture edge */
typedef enum _ftm_input_capture_edge
{
    kFTM_RisingEdge      = (1U << FTM_CSC_ELSA_SHIFT), /*!< Capture on rising edge only*/
    kFTM_FallingEdge     = (2U << FTM_CSC_ELSA_SHIFT), /*!< Capture on falling edge only*/
    kFTM_RiseAndFallEdge = (3U << FTM_CSC_ELSA_SHIFT)  /*!< Capture on rising or falling edge */
...
typedef enum _ftm_dual_edge_capture_mode
{
    ...
    kFTM_Continuous = (1U << FTM_CSC_MSA_SHIFT) /*!< Continuous capture mode */
 ...
 */
int32_t FTM_SetupPwm(FTM_Type *base,
 ...
 *         kStatus_Error on failure
 */
int32_t FTM_UpdatePwmDutycycle(FTM_Type *base,
 ...
 *         kStatus_Error on failure
 */
int32_t FTM_SetupPwmMode(FTM_Type *base,
...
{
    return (base->C[chnlNumber].CV & FTM_CV_VAL_MASK);
}
...
static inline void FTM_EnableDmaTransfer(FTM_Type *base, ftm_chnl_t chnlNumber, bool enable)
{
    ...
    {
        /* Enable DMA transfer */
        base->C[chnlNumber].CSC |= FTM_CSC_DMA_MASK;
    }
    else
    {
